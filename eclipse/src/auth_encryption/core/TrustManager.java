...
import auth_encryption.primitives.MetaInformation;
...
public final class TrustManager {
	private String TAG = TrustManager.class.getSimpleName();
	...
	private int checkValidity(Fingerprint fingerprint, boolean deleteIfUnknown) {
		...
		if (trustInfo.degree != -1 && trustInfo.degree <= AuthenticationApplication.TRUST_DEGREE
	...
	public MetaInformation getMetaInformation(Fingerprint fingerprint) {
		...
		List<String>[] priorities = new List[AuthenticationApplication.TRUST_DEGREE + 2];
		...
		for (Signature signature : subject.issuers) {
			...
			int i = AuthenticationApplication.TRUST_DEGREE + 1;
			if (this.subjects.containsKey(signature.getIssuer())) {
				Subject issuer = this.subjects.get(signature.getIssuer());
		...
		for (int i = 0; i < AuthenticationApplication.TRUST_DEGREE + 2; i++) {
	...
	public int measureNumDirectTrustRelations() {
		// without reflexive trust relation
		return this.owner.issued.size() - 1;
	}

	public int measureNumIndirectTrustRelations() {
		// without reflexive trust relations
		// all subjects in repository are at least KNOWN!
		return this.subjects.size() - this.measureNumDirectTrustRelations() - 1;
	}
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		TrustManager other = (TrustManager) obj;
		if (basePath == null) {
	...
	public String debugCompare(TrustManager other) {
		Set<Fingerprint> missingSubjects1 = new HashSet<>();
		List<String> differentSubjects = new ArrayList<>();
		for (Subject s1 : this.subjects.values()) {
			if (!other.subjects.containsKey(s1.fingerprint)) {
				missingSubjects1.add(s1.fingerprint);
				continue;
			}

			Subject s2 = other.subjects.get(s1.fingerprint);
			if (s1.equals(s2))
				continue;

			differentSubjects.add(this.debugCompareSubject(s1, s2));
		}

		Set<Fingerprint> missingSubjects2 = new HashSet<>();
		for (Subject s1 : other.subjects.values()) {
			if (!this.subjects.containsKey(s1.fingerprint)) {
				missingSubjects2.add(s1.fingerprint);
				continue;
			}
		}
		...
		result.append("+ " + missingSubjects1 + "\n");
		result.append("- " + missingSubjects2 + "\n");

		result.append("# Different\n");
		for (String diff : differentSubjects)
			result.append(diff);

		return result.toString();
	}

	private String debugCompareSubject(Subject s1, Subject s2) {
		StringBuilder result = new StringBuilder();

		result.append("~ " + s1.fingerprint + "\n");

		if (!s1.trustInfo.equals(s2.trustInfo)) {
			result.append("\t" + s1 + ", " + s2 + "\n");
			result.append("\t-\n");
		}

		result.append(this.debugCompareSets(s1.issuers, s2.issuers));
		result.append("\t-\n");

		result.append(this.debugCompareSets(s1.issued, s2.issued));
		result.append("\t-\n");

		result.append(this.debugCompareSets(s1.subKeys.keySet(), s2.subKeys.keySet()));

		return result.toString();
	}

	private String debugCompareSets(Set<?> s1, Set<?> s2) {
		if (s1.equals(s2))
			return "";

		StringBuilder result = new StringBuilder();

		Set<?> k1 = new HashSet<>(s1);
		k1.removeAll(s2);
		result.append("\t+ " + k1 + "\n");

		Set<?> k2 = new HashSet<>(s2);
		k2.removeAll(s1);
		result.append("\t- " + k2 + "\n");
