...
public class SimScenario implements Serializable {
	 ...
	 */
	protected void createHosts() throws Exception{ ///AUTH
		this.hosts = new ArrayList<DTNHost>();
		
		/**
		 * START AUTHENTICATION
		 * */
		Settings gs = new Settings(GROUP_NS);
		String app = gs.getSetting(GAPPNAME_S+1);
		Settings apps = new Settings(app);
		
		TRUST_DEGREE = apps.getInt(TRUST);
		NUMBEROFRUNS = apps.getInt(RUNS);
		if(apps.getBoolean(KEYPARM))
		...
		SimulationConfig.SIMULATION_PATH = "simulation"+suffix+"/";
		
		//FileUtils.deleteDirectory(new File(SimulationConfig.SIMULATION_PATH));
		...
		for (int i=1; i<=nrofGroups; i++) {
			...
			for (int j=0; j<nrofHosts; j++) {
				...
				do{
					uniqueUUID = true;
					uuid = SimulationUtils.getUUID();
					for(DTNHost current : this.hosts){
				...
				DTNHost host = new DTNHost(this.messageListeners,
						this.movementListeners,	gid, interfaces, comBus,
						mmProto, mRouterProto, uuid);
				host.initializeProtocol();
				host.generateSubKeys();
				host.recordMetrics();
				//////////////////////////////////////////////	
				/**
				 * END AUTHENTICATION
